...
#define DEFINE_RING_TYPE(type)                                                  \
    ...
    static type ## _ring_t ring_init_ ## type(unsigned capacity) {              \
        return (type ## _ring_t) {                                              \
            ...
            malloc(capacity * sizeof(type))                                     \
                                                                                ...
                                                                                \
    static type* ring_front_ ## type(type ## _ring_t* ring) {                   \
        if(ring_empty(*ring))                                                   \
            return nullptr;                                                     \
    ...
    static void ring_deinit_ ## type(type ## _ring_t* ring) {                   \
        ...
        ring->next = 0;                                                         \
                                                                                ...
                                                                                \
    static type* ring_push_back_ ## type(type ## _ring_t* ring) {               \
        ...
        return &ring->data[pop];                                                \
                                                                                ...
                                                                                \
    static bool ring_pop_back_ ## type(type ## _ring_t* ring, type* popped) {   \
        if(ring_empty(*ring))                                                   \
            return false;                                                       \
        unsigned last = (ring->next - 1) % ring->capacity;                      \
        if(popped != nullptr)                                                   \
        ...
        return true;                                                            \
                                                                                ...
                                                                                \
    static bool ring_pop_front_ ## type(type ## _ring_t* ring, type* popped) {  \
        if(ring_empty(*ring))                                                   \
            return false;                                                       \
        if(popped != nullptr)                                                   \
